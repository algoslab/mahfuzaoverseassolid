...
<div class="box">
    ...
    <div class="box-header with-border d-flex justify-content-between align-items-center">
        <div>
            ...
            <h6 class="box-subtitle">This is all Countries List</h6>
        </div>
    ...
    @include('supper_admin.components.location.country_modal')
            ...
            <table id="customDataTable" style="table-layout: fixed; width: 100%;" class="table table-bordered table-hover display nowrap margin-top-10 w-p100">
                <thead>
                    <tr>
                        ...
                        <th style="">Code</th>
                        <th style="">Phone Code</th>
                        <th style="">Status</th>
                        <th style="">Created At</th>
                    </tr>
                </thead>
                <tbody>
                    @foreach($countries as $key =>$serivice)
                    <tr>
                        <td>
                            <div class="btn-group">
                        ...
                        <td class="wrap-text">{{ $serivice->country_code  }}</td>
                        <td class="wrap-text">{{ $serivice->phone_code  }}</td>
                        <td>
    ...
    @section('script')
        <script>
            function fetchCountries() {
                $.ajax({
                    url: '{{ route("supper_admin.countries.index") }}',
                    type: 'GET',
            ...
            $(document).ready(function () {
                ...
                $('#countryForm').on('submit', function (e) {
                    ...
                    let isEdit = $('#country_id').val() !== ''; 
                    let formData = new FormData(this); 
                    let id = $('#country_id').val(); 
                    let url = isEdit 
                        ? `{{ route('supper_admin.countries.update', ['country' => '__id__']) }}`.replace('__id__', id) 
                        : `{{ route('supper_admin.countries.store') }}`;
                ...
                $(document).on('click', '.editBlogButton', function () {
                    ...
                    const url = '{{ route("supper_admin.countries.edit", ":id") }}'.replace(':id', id);
                ...
                $(document).on('click', '.deletecountryBtn', function () {
                    const id = $(this).data('id');
                    const url = '{{ route("supper_admin.countries.destroy", ":id") }}'.replace(':id', id);
