...
<div class="box">
    ...
    <div class="box-header with-border d-flex justify-content-between align-items-center">
        <div>
            ...
            <h6 class="box-subtitle">This is all Designation List</h6>
        </div>
    ...
    @include('backend.components.myOffice.designation_modal')
            ...
            <table id="customDataTable" style="table-layout: fixed; width: 100%;" class="table table-bordered table-hover display nowrap margin-top-10 w-p100">
                <thead>
                    <tr>
                        ...
                        <th style="">Designation Code</th>
                        <th style="">Note</th>
                        <th style="">Status</th>
                    </tr>
                </thead>
                <tbody>
                    @foreach($designations as $key =>$serivice)
                    <tr>
                        <td>
                            <div class="btn-group">
                                ...
                                <div class="dropdown-menu">
                                    ...
                                    <button type="button"
                                            class="dropdown-item text-danger deletedesignationBtn"
                                            data-id="{{ $serivice->id }}">
        ...
        <script>
            function fetchdesignation() {
                $.ajax({
                    ...
                    error: function () {
                        console.error('Failed to refresh designation table.');
                    }
                });
            }
            ...
            $(document).ready(function () {
                $('#designationFrom').on('submit', function (e) {
                    e.preventDefault();
                    let isEdit = $('#designation_id').val() !== '';
                    let formData = new FormData(this);
                    let id = $('#designation_id').val();

                    let url = isEdit
                        ? `{{ route('admin.designations.update', ['designation' => '__id__']) }}`.replace('__id__', id)
                ...
                $(document).on('click', '.editBlogButton', function () {
                    ...
                    const url = '{{ route("admin.designations.edit", ":id") }}'.replace(':id', id);
                    ...
                    $.ajax({
                        ...
                        success: function (res) {
                            ...
                            $('#modal-center').modal('show');
                            $('#countriesSelect').val(res.designation_id).trigger('change');
                        },
                        error: function () {
                ...
                $(document).on('click', '.deletedesignationBtn', function () {
                    const id = $(this).data('id');
                    const url = '{{ route("admin.designations.destroy", ":id") }}'.replace(':id', id);
