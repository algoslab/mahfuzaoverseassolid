...
<div class="box">
    ...
    <div class="box-header with-border d-flex justify-content-between align-items-center">
        <div>
            ...
            <h6 class="box-subtitle">This is all role List</h6>
        </div>
        <button type="button" class="btn btn-warning addRoleButton" data-toggle="modal" data-target="#modal-center">
            <i class="fa-solid fa-plus"></i> Add Data
        </button>
    </div>

    @include('backend.components.myOffice.role_modal')
            ...
            <table id="customDataTable" style="table-layout: fixed; width: 100%;" class="table table-bordered table-hover display nowrap margin-top-10 w-p100">
                <thead>
                    <tr>
                        ...
                        <th style="">Role Code</th>
                        <th style="">Note</th>
                        <th style="">Status</th>
                    </tr>
                </thead>
                <tbody>
                    @foreach($roles as $key =>$serivice)
                    <tr>
                        <td>
                            <div class="btn-group">
                                ...
                                <div class="dropdown-menu">
                                    ...
                                    <button type="button"
                                            class="dropdown-item text-danger deleteroleBtn"
                                            data-id="{{ $serivice->id }}">
        ...
        <script>
            function fetchRole() {
                $.ajax({
                    ...
                    error: function () {
                        console.error('Failed to refresh role table.');
                    }
                });
            }
            ...
            $(document).ready(function () {
                $('#roleFrom').on('submit', function (e) {
                    e.preventDefault();
                    let isEdit = $('#role_id').val() !== '';
                    let formData = new FormData(this);
                    let id = $('#role_id').val();

                    let url = isEdit
                        ? `{{ route('admin.roles.update', ['role' => '__id__']) }}`.replace('__id__', id)
                ...
                $(document).on('click', '.editBlogButton', function () {
                    ...
                    const url = '{{ route("admin.roles.edit", ":id") }}'.replace(':id', id);
                    ...
                    $.ajax({
                        ...
                        success: function (res) {
                            ...
                            $('#modal-center').modal('show');
                            $('#countriesSelect').val(res.role_id).trigger('change');
                        },
                        error: function () {
                ...
                $(document).on('click', '.deleteroleBtn', function () {
                    const id = $(this).data('id');
                    const url = '{{ route("admin.roles.destroy", ":id") }}'.replace(':id', id);
