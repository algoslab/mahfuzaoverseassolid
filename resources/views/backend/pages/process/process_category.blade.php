...
@section('title', config('app.name') . ' - Process Category')
...
<div class="box">
    ...
    <div class="box-header with-border d-flex justify-content-between align-items-center">
        <div>
            <h3 class="box-title">Process Categorys</h3>
            <h6 class="box-subtitle">This is all Process Category List</h6>
        </div>
        <button type="button" class="btn btn-warning addProcessCategoryButton" data-toggle="modal" data-target="#modal-center">
            <i class="fa-solid fa-plus"></i> Add Data
        </button>
    </div>

    @include('backend.components.process.process_category_modal')
    ...
    <div class="box-body">
        <div class="table-responsive">
            <table id="customDataTable" style="table-layout: fixed; width: 100%;" class="table table-bordered table-hover display nowrap margin-top-10 w-p100">
                ...
                <tbody>
                    @foreach($ProcessCategorys as $key =>$serivice)
                    <tr>
                        <td>
                            <div class="btn-group">
    ...
    @section('script')
        <script>
            function fetchCandidateType() {
                $.ajax({
                    url: '{{ route("admin.processCategory.index") }}',
                    type: 'GET',
            ...
            $(document).ready(function () {
                $('#processCategoryForm').on('submit', function (e) {
                    e.preventDefault();
                    let isEdit = $('#process_category_id').val() !== '';
                    let formData = new FormData(this);
                    let id = $('#process_category_id').val();

                    let url = isEdit
                        ? `{{ route('admin.processCategory.update', ['processCategory' => '__id__']) }}`.replace('__id__', id)
                        : `{{ route('admin.processCategory.store') }}`;
                    ...
                    Swal.fire({
                        title: isEdit ? "Update Process Category?" : "Add Process Category?",
                        icon: "question",
                            ...
                            $.ajax({
                                ...
                                success: function (response) {
                                    if (response.status === 'success') {
                                        ...
                                        $('#processCategoryForm')[0].reset();
                                        $('#process_category_id').val('');
                                        fetchCandidateType();
                                    } else {
                ...
                $(document).on('click', '.addProcessCategoryButton', function () {
                    $('#processCategoryForm')[0].reset();
                    $('#process_category_id').val('');
                ...
                $(document).on('click', '.editBlogButton', function () {
                    ...
                    const url = '{{ route("admin.processCategory.edit", ":id") }}'.replace(':id', id);
                    ...
                    $.ajax({
                        ...
                        success: function (res) {
                            console.log(res);
                            $('#process_category_id').val(id);
                            $('#name').val(res.name);
                            $('#note').val(res.note);
                            $('#status').prop('checked', res.status == 1);
                            $('#modalTitle').text('Edit Process Category');
                            $('#modal-center').modal('show');
                            $('#countriesSelect').val(res.process_category_id).trigger('change');
                        },
                        error: function () {
